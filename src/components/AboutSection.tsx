...
const AboutSection = () => {
  return (
    <div className="grid grid-cols-1 md:grid-cols-1 lg:grid-cols-2 p-4 sm:p-8 lg:p-16 *:font-inter relative">
      <div className="grid grid-cols-1 gap-3 mb-8 lg:mb-0">
        <p className="text-[#EB8D15] font-inter tracking-[1.5px]">
          WELCOME TO REGISTERKARO.IN
        </p>
        <h3 className="text-[24px] md:text-[28px] lg:text-[32px] font-bold text-[#272D37]">
          About <span className="text-[#EB8D15]">Register Karo</span>
        </h3>
        <p className="text-base leading-6 tracking-wide text-[#0D1216]">
          {`We have been using Intelegencia as our DevOps vendor for our field
      ...
      <div className="flex flex-1 items-center justify-center md:mt-8 lg:mt-0 lg:pl-8">
        <img src={PeopleImage} alt="" className="w-full max-w-[584px]" />
      </div>
      <img
        src={Dot}
        alt=""
        className="-z-10 absolute right-3 bottom-[68px] w-[80px] md:w-[100px] lg:w-[130px] hidden sm:block"
      />
      <img
        src={Curve}
        alt=""
        className="absolute h-[300px] md:h-[400px] lg:h-[500px] top-[30px] right-0 hidden sm:block"
      />
    </div>
  );
};
